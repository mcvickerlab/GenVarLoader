...
- `"variant-windows"` / `"reference"` output kinds and non-default `var_fields` remain later **Wave B (issue #304)** follow-ups (PRs B3–B4) — `with_seqs` raises `NotImplementedError` for `"variant-windows"`/`"reference"`, and streaming non-default `var_fields` are not yet wired. `"variants"` output is wired as of Wave B PR-B1, and `min_af`/`max_af` filtering as of Wave B PR-B2 — see below.

**`with_seqs("variants")` (Wave B PR-B1, #304) — SVAR1, VCF, and PGEN backends:** returns `RaggedVariants` with the default field set (`alt`, `start`, `ilen`) — same defaults as the written `Dataset.open(...).with_seqs("variants")`. Variants are clipped to each region's read window (the same overlap-clip the written path applies), and output is byte-identical to `Dataset.open(...).with_seqs("variants")[r, s]` at `jitter=0`. Unlike the written path, streamed variant records carry no dataset-global variant id — each `RaggedVariants` entry is self-contained. Non-default `var_fields` (e.g. `dosage`/FORMAT columns) on the streaming variants path is not yet wired (later Wave B PR-B3). The `.svar2` backend does not support `"variants"` streaming — it remains haplotypes-only (see above).
