...
- `progress_callback`: a `gvl.ProgressCallback`, receiving every immutable `gvl.ProgressEvent(phase, completed, total, unit, state, message)` exactly. Events reject blank labels, non-integer counts (including booleans), and unknown states. `complete` requires a known total with `completed == total`; all other states remain below a known total. `progress_path` selects a `gvl.JsonProgressSink` atomic JSON snapshot. Standalone or partial-identity sinks use generic `schema_version=1`, `source="genvarloader"` JSON; a positive attempt plus every managed identity field selects `nf-seqlab.progress/v1`, with a missing/blank parent ID normalized to the file ID. If the path is omitted, `NF_SEQLAB_PROGRESS_PATH`, `_FILE`, `_SNAPSHOT_PATH`, `_DIR`, or the `NF_SEQLAB_PROGRESS` shorthand selects it. Structured progress disables native `tqdm`; observer failures are logged and isolated. Snapshot publication is serialized per sink; running writes are coalesced to at most four per second, while terminal updates are immediate, final, and ignore later events. In-flight events reserve 100, and successful completion is emitted only after this writer publishes the dataset and re-reads `metadata.json`; failed/cancelled terminals retain the known total but cap `completed` below it. Snapshot paths at or below the dataset, the reserved `<path>.lock` sibling, and anything below that lock path are rejected; the lock sibling is derived before resolving destination symlinks, matching publication locking.
...
**Atomic creation:** `gvl.write` builds into a private sibling temp directory. New destinations publish with one atomic `os.replace`; overwrite uses rollback-safe move-aside-then-rename, so readers can briefly observe the destination as absent. A failed publish restores the prior directory; if rollback also fails, the prior directory remains under a sibling `.old.*` backup. A best-effort `filelock` avoids redundant rebuilds when parallel jobs share the same destination but is advisory only. **Datasets do not auto-rebuild**; if the on-disk artifact is missing or corrupt, re-run `gvl.write`.
...
- `gvl.Reference.from_path(fasta, contigs=None)` — wrap a FASTA (path to a `.fa`/`.fa.bgz`, or a `.gvlfa` cache dir). Builds/reuses a sibling `.gvlfa` cache directory (self-describing, fingerprint-validated; legacy `.fa.gvl` caches auto-migrate). Legacy migration holds a source-specific lock, verifies an opened source descriptor against the captured identity, copies only from that descriptor, then atomically renames the public source to a private cleanup candidate. Immediately after acquiring the lock, queued migrators reuse a matching publication before inspecting any still-present legacy path. Rollback preserves the original source and destination, and replacement files are restored without clobbering newer paths. After publication commits, cleanup errors only warn and preserve recoverable bytes at the public source, private candidate, or both. The cache is built atomically (temp + `os.replace`) under a best-effort lock, so concurrent builders sharing one reference are safe; the cache **auto-rebuilds** from its source when stale or missing.
