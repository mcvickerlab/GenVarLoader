...
- `"variant-windows"` / `"reference"` output kinds, `min_af`/`max_af`, and non-default `var_fields` remain later **Wave B (issue #304)** follow-ups (PRs B2–B4) — `with_seqs` raises `NotImplementedError` for `"variant-windows"`/`"reference"`, and streaming `min_af`/`max_af`/non-default `var_fields` are not yet wired. `"variants"` output is wired as of Wave B PR-B1 — see below.

**`with_seqs("variants")` (Wave B PR-B1, #304) — SVAR1, VCF, and PGEN backends:** returns `RaggedVariants` with the default field set (`alt`, `start`, `ilen`) — same defaults as the written `Dataset.open(...).with_seqs("variants")`. Variants are clipped to each region's read window (the same overlap-clip the written path applies), and output is byte-identical to `Dataset.open(...).with_seqs("variants")[r, s]` at `jitter=0`. Unlike the written path, streamed variant records carry no dataset-global variant id — each `RaggedVariants` entry is self-contained. `min_af`/`max_af` filtering and non-default `var_fields` (e.g. `dosage`/FORMAT columns) on the streaming variants path are not yet wired (later Wave B PRs B2–B4). The `.svar2` backend does not support `"variants"` streaming — it remains haplotypes-only (see above).
...
- `min_af` / `max_af` are supported for `.svar` only. A non-SVAR-backed dataset (plain BCF/PGEN) raises `RuntimeError`; a `.svar2`-backed dataset raises `NotImplementedError` specifically.
...
- **`gvl.StreamingDataset` is iterable-only.** `sds[r, s]` raises `TypeError` — use `sds.to_iter(...)` (or `to_dataloader(...)` for torch); there is no `__iter__`. `variants=` accepts `.svar`, `.svar2`, VCF/BCF (`.vcf[.gz]`/`.bcf`), or PGEN (`.pgen`, biallelic only — multiallelic raises at construction). `with_seqs("variants")` is supported on the SVAR1, VCF, and PGEN backends as of Wave B PR-B1 (#304), byte-identical to the written path (see above); `with_seqs` kinds `"variant-windows"`/`"reference"` (still Wave B, PRs B2–B4), `with_len("variable")`, and `num_workers > 0` all raise `NotImplementedError`/`ValueError`, as do streaming `min_af`/`max_af` and non-default `var_fields` (also later Wave B). `with_len(int | "ragged")`, `with_settings(jitter=, rng=, deterministic=)`, `with_seqs("annotated")`, and `with_seqs("variants")` are supported for the SVAR1, VCF, and PGEN backends; the `.svar2` backend is currently haplotypes-only, `jitter=0`, ragged output only (combining a `.svar2` source with `jitter>0`, `with_len(<int>)`, `with_seqs("annotated")`, or `with_seqs("variants")` raises `NotImplementedError`). See "`gvl.StreamingDataset` — write-free streaming (SVAR1 + SVAR2 + VCF/BCF + PGEN)" above.
