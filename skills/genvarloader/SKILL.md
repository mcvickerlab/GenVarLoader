...
- `"variants"` / `"variant-windows"` / `"reference"` output kinds, `min_af`/`max_af`, and `var_fields` are **Wave B (issue #304)** — not in Wave A; `with_seqs` raises `NotImplementedError` for any kind other than `"haplotypes"`/`"annotated"`.
...
- **`gvl.StreamingDataset` is iterable-only.** `sds[r, s]` raises `TypeError` — use `sds.to_iter(...)` (or `to_dataloader(...)` for torch); there is no `__iter__`. `variants=` accepts `.svar`, `.svar2`, VCF/BCF (`.vcf[.gz]`/`.bcf`), or PGEN (`.pgen`, biallelic only — multiallelic raises at construction). `with_seqs` kinds other than `"haplotypes"`/`"annotated"` (i.e. `"variants"`/`"variant-windows"`/`"reference"`, Wave B #304), `with_len("variable")`, and `num_workers > 0` all raise `NotImplementedError`/`ValueError`. `with_len(int | "ragged")`, `with_settings(jitter=, rng=, deterministic=)`, and `with_seqs("annotated")` are supported (Wave A, #277) for the SVAR1, VCF, and PGEN backends; the `.svar2` backend is currently haplotypes-only, `jitter=0`, ragged output only (combining a `.svar2` source with `jitter>0`, `with_len(<int>)`, or `with_seqs("annotated")` raises `NotImplementedError`). See "`gvl.StreamingDataset` — write-free streaming (SVAR1 + SVAR2 + VCF/BCF + PGEN)" above.
