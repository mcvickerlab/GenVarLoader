...
**`with_seqs("variant-windows", opt)` (Wave B PR-B4, #304) — SVAR1, VCF, and PGEN backends:** `opt` (a `gvl.VarWindowOpt`) is **required** for this kind and rejected (`ValueError`) for every other `with_seqs` kind. Unlike the written path's `Dataset.with_seqs("variant-windows", opt)` (which returns a `FlatVariantWindows`), streaming returns a plain `dict[str, Ragged]` per cell — keys `start`/`ilen` (one ragged axis, `(b, p, ~v)`) plus whichever of `ref_window`/`alt_window` (mode `"window"`) or `ref`/`alt` (mode `"allele"`) `opt.ref`/`opt.alt` selected (two ragged axes, `(b, p, ~v, ~w)` — variant count AND per-variant window/allele length are both ragged). Token dtype (`uint8` or `int32`, from `build_token_lut`) is preserved exactly, never coerced. The SVAR2 (`.svar2`) backend is **not** supported — `with_seqs("variant-windows", opt)` raises `NotImplementedError` immediately (at config time, not iterate time) for a `.svar2`-backed `StreamingDataset`. `var_fields`/`min_af`/`max_af` are not combinable with `"variant-windows"` (same `NotImplementedError` guard as every other non-`"variants"` kind); `VarWindowOpt` has no `var_fields`-equivalent knob of its own for streaming yet.
