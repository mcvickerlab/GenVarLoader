...
## `gvl.StreamingDataset` — write-free streaming (SVAR1 + VCF/BCF + PGEN)

`gvl.StreamingDataset(regions, reference=..., variants=...)` iterates haplotype batches directly off a live variant source — **no `gvl.write()` step, no on-disk `.gvl` dataset**. `variants=` accepts a `.svar` (SparseVar/SVAR1) store directory, a VCF/BCF path (`.vcf`, `.vcf.gz`/`.vcf.bgz`, `.bcf`; indexed, same as `gvl.write`), or a PGEN file-set (`.pgen` + sibling `.pvar` + `.psam`, same trio `gvl.write` expects; pass the `.pgen` path or a bare plink2 prefix). All three read paths go through a shared Rust `RecordStreamEngine` (a generic detached-producer/consumer engine core); VCF/BCF decodes each window via genoray's Rust `VcfRecordSource → ChunkAssembler → DenseChunk` pipeline, PGEN via `PgenRecordSource → ChunkAssembler → DenseChunk` (same transpose downstream — `DenseChunk` is source-agnostic). Haplotype (and `with_seqs("annotated")`) output is byte-identical to `gvl.write(...)` + `Dataset.open(...)[r, s]` at `jitter=0`, for all three backends. `with_len`, `with_settings(jitter=, rng=, deterministic=)`, and `with_seqs("annotated")` are all available — see "Output-mode breadth (Wave A, #277)" below.
...
**Output-mode breadth (Wave A, #277) — `with_len`, jitter, and `with_seqs("annotated")`:**
...
- Variant source: `.svar` (SparseVar/SVAR1), VCF/BCF, or PGEN (biallelic only). `.svar2` inputs raise `NotImplementedError`.
...
- **`gvl.StreamingDataset` is iterable-only.** `sds[r, s]` raises `TypeError` — use `sds.to_iter(...)` (or `to_dataloader(...)` for torch); there is no `__iter__`. `variants=` accepts `.svar`, VCF/BCF (`.vcf[.gz]`/`.bcf`), or PGEN (`.pgen`, biallelic only — multiallelic raises at construction); `.svar2` variant sources, `with_seqs` kinds other than `"haplotypes"`/`"annotated"` (i.e. `"variants"`/`"variant-windows"`/`"reference"`, Wave B #304), `with_len("variable")`, and `num_workers > 0` all raise `NotImplementedError`/`ValueError`. `with_len(int | "ragged")`, `with_settings(jitter=, rng=, deterministic=)`, and `with_seqs("annotated")` are supported (Wave A, #277) — see "`gvl.StreamingDataset` — write-free streaming (SVAR1 + VCF/BCF + PGEN)" above.
