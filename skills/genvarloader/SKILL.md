...
- `gvl.Ragged`, `gvl.RaggedAnnotatedHaps`, `gvl.RaggedVariants`, `gvl.RaggedIntervals` — ragged return containers. All are backed by `seqpro.rag.Ragged` (`_core.Ragged` Rust+numba backend); **not** `awkward`. `RaggedVariants` is a thin wrapper over a single record `seqpro.rag.Ragged` with opaque-string `alt`/`ref` fields. Named properties (`.alt`, `.ref`, `.start`, `.ilen`, `.end`) are the primary access point; extra fields (e.g. `AF`, custom FORMAT fields) are also accessible via `rv["field"]` or `rv.field` (via `__getattr__`). Equality (`==`) comes from `_core.Ragged` ufunc-based comparison, not awkward. Public method surface: `.rc_()`, `.pad()`, `.to_packed()`, `.reshape()`, `.squeeze()`, `.to_nested_tensor_batch()`.
