...
def parse_writefasta(args):
    ...
    if args.encode:
        write_encoded_genome(
            ...
            encode_spec=args.spec,
        )
    else:
        write_genome_seq(
            args.input, args.directory, h5_name=args.name, chrom_list=args.chroms
        )
...
def parse_cmd():
    ...
    writefa_parser.add_argument(
        ...
        "--onehot",
        action="store_true",
        help="Write onehot-encoded genome",
    )
    writefa_parser.add_argument(
        "-s",
        "--spec",
    ...
    frag_parser.add_argument(
        "--ignore_offset", action="store_true", help="Don't offset tn5 cutsites"
    )
    frag_parser.add_argument(
        "-m",
        "--method",
        "--count_method",
        type=str,
        choices=["cutsite", "midpoint", "fragment"],
        default="cutsite",
