...
def writefasta(
    ...
    if (chroms is not None) and (len(chroms) == 1):
...
def writefrag(
    ...
    lens: Optional[List[str]] = typer.Option(
        None,
        "-l",
        "--lens",
        "--lengths",
    ...
    ignore_offset: bool = typer.Option(
        False, "--ignore_offset", help="Don't offset tn5 cutsites"
    ),
    count_method: str = typer.Option(
        "cutsite",
        "--method",
        help="Counting method, Choice of 'cutsite', 'midpoint, 'fragment'",
    ),
):
    if output.suffix not in {".h5", ".hdf5", ".hdf", ".he5"}:
        output.suffix = ".h5"
    ...
    if count_method not in {"cutsite", "midpoint", "fragment"}:
        typer.echo(
            "Please input valid count method! ('cutsite', 'midpoint, 'fragment')"
        )
        raise typer.Exit(code=1)
