...
class ConsensusGenomeLoader:
    ...
    def __init__(self, ref_genome_h5: PathType, vcf_zarr: PathType) -> None:
        ...
        with self._open() as ref_genome:
            self.ref_embedding: str = ref_genome.attrs["id"]
            logger.info(f"Genome is {self.ref_embedding} embedded.")
    ...
    def sel(
        ...
        starts: NDArray[np.signedinteger],
        length: Union[int, np.unsignedinteger],
        samples: Optional[
            Union[list[str], NDArray[np.str_], NDArray[np.unsignedinteger]]
        ] = None,
        strands: Optional[NDArray[np.str_]] = None,
        sorted_contigs: bool = False,
        pad_val: Union[bytes, str] = "N",
        ploid_idx: Optional[NDArray[np.unsignedinteger]] = None,
        ...
        starts, ends, length = self._sel_get_regions(starts, length)
        pad_arr = self._sel_get_pad_arr(pad_val)
        ...
        if samples is not None:
            ...
            if isinstance(samples, np.ndarray) and samples.dtype.kind == "u":
                ...
                sample_idx: NDArray[np.unsignedinteger] = samples  # type: ignore
            else:
            ...
            if self.ref_embedding not in valid_embeddings:
                raise ValueError(
    ...
    def _sel_get_regions(
        self, starts: NDArray[np.signedinteger], length: Union[int, np.unsignedinteger]
    ):
        length = np.uint32(length)
        ends = starts + length
        return starts, ends, length

    def _sel_get_pad_arr(self, pad_val: Union[bytes, str]):
        if isinstance(pad_val, str):
            pad_val = bytes(pad_val, "utf-8")
        if self.ref_embedding == "sequence":
            pad_arr = np.array([pad_val], dtype="|S1")
        elif self.ref_embedding == "onehot":
            assert self.spec is not None
    ...
    def _sel_slice(
        ...
        for chrom in np.unique(contigs):  # guaranteed to proceed in a sorted order
            ...
            ref_chrom, ref_start, rel_starts = self._sel_ref_chrom(
    ...
    def _sel_ref_chrom(
        ...
        """Load a slice of chromosome of reference nucleotides into memory that spans the query regions.
        Will add padding for any regions outside the reference sequence.

        Parameters
        ----------
        ref_chrom_h5 : h5py.Group
            HDF5 group for the reference chromosome.
        starts : NDArray[np.int32]
            Starts of each regions.
        ends : NDArray[np.int32]
            Ends of each region.
        pad_arr : Union[NDArray[np.bytes_], NDArray[np.uint8]]
            Array to use for padding. Shape: (1,) for np.bytes_, (alphabet,) for np.uint8
        ...
        # pad_arr shape: (1) or (alphabet)
        ref_chrom: Union[NDArray[np.bytes_], NDArray[np.uint8]]
        ref_chrom = ref_chrom_h5[self.ref_embedding][real_ref_start:real_ref_end]
...
class FixedLengthConsensus:
    ...
    def from_gl_bed(
        ...
        z.attrs["embedding"] = gl.ref_embedding
        ...
        if max_memory is None:
            ...
            if gl.ref_embedding == "onehot":  # assume byte sequence otherwise
                chunks.append(None)
