...
def get_frag_depth(
    in_bam, chrom_list=None, chrom_lens=None, offset_tn5=True, count_method="cutsite"
):
    ...
    with AlignmentFile(in_bam, "r") as bam:
        ...
        if not chrom_list or not chrom_lens:
            ...
            chrom_lens = {
                chrom: bam.header.get_reference_length(chrom) for chrom in chrom_list
            }
        ...
        for chrom in chrom_list:
            ...
            print(
                f"Counted {chrom} fragments in {timeit.default_timer() - start_chrom:.2f} seconds!"
            )

    print(
        f"Processed fragment data in {timeit.default_timer() - start_time:.2f} seconds!"
    )
    return depth_dict
