    ...
    if sample:
        with VariantFile(in_vcf, "r") as vcf:
            ...
            snp_df = pd.DataFrame(
                [
                    record
                    for record in vcf_snp_gen(vcf, chrom_list=chrom_list, sample=sample)
                ],
        ...
        with VariantFile(in_vcf, "r", drop_samples=True) as vcf:
            ...
            snp_df = pd.DataFrame(
                [record for record in vcf_snp_gen(vcf, chrom_list=chrom_list)],
    ...
    if sample:
        with VariantFile(in_vcf, "r") as vcf:
            ...
            snp_df = (
                pl.DataFrame(
                    [
                        record
                        for record in vcf_snp_gen(
                            vcf, chrom_list=chrom_list, sample=sample
                        )
                    ],
                    columns=[
                        "chrom",
        ...
        with VariantFile(in_vcf, "r", drop_samples=True) as vcf:
            ...
            snp_df = (
                ...
                .lazy()
                .with_columns(
                    [pl.col("start").cast(pl.UInt32), pl.col("stop").cast(pl.UInt32)]
                )
                .collect()
            )
