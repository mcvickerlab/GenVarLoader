...
- **Write-free streaming:** `gvl.StreamingDataset` reads haplotypes straight from a VCF/BCF or `.svar` store — no `gvl.write()` step, no on-disk dataset (see the [FAQ](https://genvarloader.readthedocs.io/en/latest/faq.html))
...
The wheel statically links htslib (via [genoray](https://github.com/mcvickerlab/genoray)'s Rust core) to read VCF/BCF directly for both `gvl.write` and `gvl.StreamingDataset` — no separate htslib install is required.
