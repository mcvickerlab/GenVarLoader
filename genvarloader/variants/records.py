...
class Records:
    ...
    def vidx_vars_in_range(
        ...
        for contig in unique_contigs:
            ...
            _s_idxs = np.searchsorted(self.v_ends[contig], starts)
            rel_s_idxs = self.e2s_idx[contig][_s_idxs]
            rel_e_idxs = np.searchsorted(self.v_starts[contig], ends)

            for i, r_s, r_e in zip(idx, rel_s_idxs, rel_e_idxs):
                positions[i] = self.v_starts[contig][r_s:r_e]
                size_diffs[i] = self.v_diffs[contig][r_s:r_e]
                refs[i] = self.ref[contig][r_s:r_e]
                alts[i] = self.alt[contig][r_s:r_e]

            # make idxs absolute
            s_idxs = rel_s_idxs + self.contig_offsets[contig]
            e_idxs = rel_e_idxs + self.contig_offsets[contig]
            start_idxs[idx] = s_idxs
            end_idxs[idx] = e_idxs

        n_var_per_region = end_idxs - start_idxs
        offsets = np.empty(len(n_var_per_region) + 1, dtype=np.uint32)
        offsets[0] = 0
        offsets[1:] = np.cumsum(n_var_per_region)
        ...
        return RecordInfo(
            positions=positions,
            size_diffs=size_diffs,
            refs=VLenAlleles.concat(*refs),
            alts=VLenAlleles.concat(*alts),
            start_idxs=start_idxs,
            end_idxs=end_idxs,
            offsets=offsets,
        )

    def vidx_vars_in_range_for_haplotype_construction(
        self,
        contigs: ArrayLike,
        starts: ArrayLike,
        length: int,
        ...
        unique_contigs = np.unique(contigs)
        max_ends = np.empty(n_queries, np.int32)
        for contig in unique_contigs:
            idx = np.nonzero(contigs == contig)[0]
            ...
            rel_s_idxs = self.e2s_idx[contig][_s_idxs]
            rel_e_idxs = self.e2s_idx[contig][_e_idxs]
