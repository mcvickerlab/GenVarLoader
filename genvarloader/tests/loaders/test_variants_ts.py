...
import genvarloader
from genvarloader.loaders._variants_ts import Variants
...
def var_loader():
    return Variants(
        "/cellar/users/dlaub/repos/genome-loader/sbox/ccle_wes_snp.reduced.zarr"
    )
...
def test_var_loader(var_loader: Variants, sel_args: Dict, wdir: Path):
...
@parametrize_with_cases("sel_args", cases=".", prefix="sel_args_")
def test_ss_var_loader(ss_var_loader: Variants, sel_args: Dict, wdir: Path):
    res = ss_var_loader.sel(**sel_args)
    vcf = VCF(str(wdir / "data" / "ccle_snp_wes.reduced.bcf"))
    if res is None:
        for query in sel_args["queries"].itertuples():
            region_string = (
