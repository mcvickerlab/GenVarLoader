...
| `docs/superpowers/plans/2026-07-18-streaming-svar2-phase2-pr1.md` | SVAR2 Phase 2 **PR 1** (fast synchronous path): PR 1a `parallel=False` streaming reconstruct + PR 1b GIL-free Rust `svar2_read_window` (genoray `find_ranges`, no rev bump). Spec: `2026-07-18-streaming-svar2-phase2-design.md`. PR 2 (super-batch parallel reconstruct) + PR 3 (gated relaxed-order pipeline) get their own plans. | ⬜ planned — issue [#278](https://github.com/mcvickerlab/GenVarLoader/issues/278) |
