...
| `docs/superpowers/specs/2026-09-08-streaming-intervals-design.md` | Interval (BigWigs/Table) streaming + variant+interval mixed scheduler. **One** `_TrackBackend` keyed on the existing `IntervalTrack` Protocol (not one per class); `tracks=` rather than the issue's `bigwigs=`; **no new Rust kernel** (`intervals_and_realign_track_fused` already takes the arrays streaming holds). `iteration_order: "auto"\|"regions"\|"samples"`, `auto` = regions (variants only) / samples (tracks only, justified by `BigWigs`' one-file-per-sample layout) / regions (mixed, documented non-optimal). `Table` is in-memory and order-indifferent; its real lever is a **blocking prerequisite** — `RustTable::build_trees` rebuilds all-sample COITrees on every `count_intervals`/`_intervals_from_offsets` call, so streaming needs the per-contig tree cache `write_track_impl` already has. | 🚧 spec written, pending review — issue [#279](https://github.com/mcvickerlab/GenVarLoader/issues/279) |
