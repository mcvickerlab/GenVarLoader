...
| `docs/superpowers/specs/2026-09-08-streaming-intervals-design.md` | Interval (BigWigs/Table) streaming + variant+interval mixed scheduler. **One** `_TrackBackend` keyed on the existing `IntervalTrack` Protocol (not one per class); `tracks=` rather than the issue's `bigwigs=`. **Mixed variants+tracks shipped for SVAR1 in v1; SVAR2 (Track A) and VCF/PGEN (Track B) followed in [#375](https://github.com/mcvickerlab/GenVarLoader/issues/375)** — every variant backend now supports it; tracks-only works everywhere. No new Rust kernel, but the mixed path needs a Python-side `variant_idxs` memmap plus a window-local `geno_offset_idx` re-base. Track axis is **name-sorted** (matching the written path's `available_tracks.sort()`), and is never squeezed for a single track; rank is `(b,t,p,~l)` under `realign_tracks=True`, `(b,t,~l)` otherwise. `iteration_order: "auto"\|"regions"\|"samples"` — but a **no-op at default `max_mem`**, since `_window_samples` is derived from `max_mem` and equals `n_samples` below ~8.4M; folding track memory into that budget (§2.5) is what makes it bite. `Table` is in-memory and order-indifferent; **blocking prerequisite** — `RustTable::build_trees` rebuilds all-sample COITrees on every query call (linear in region windows, worst at cohort scale), needing the per-contig `Mutex` cache `write_track_impl` already has. | ✅ **implemented (Tasks 1-10 all landed)** — issue [#279](https://github.com/mcvickerlab/GenVarLoader/issues/279), PR [#374](https://github.com/mcvickerlab/GenVarLoader/pull/374); see the Plans-table row below |
...
| `docs/superpowers/plans/2026-09-08-streaming-intervals.md` — issue [#279](https://github.com/mcvickerlab/GenVarLoader/issues/279) | Interval (BigWigs/Table) streaming + mixed variants+tracks scheduler. Spec: `docs/superpowers/specs/2026-09-08-streaming-intervals-design.md` | ✅ **done (Tasks 1-10 all landed)**, PR [#374](https://github.com/mcvickerlab/GenVarLoader/pull/374) — per-contig `RustTable` COITree cache (Task 1), track parity fixtures (Task 2), `_TrackBackend` window reads (Task 3), `iteration_order`/`_plan()` region-vs-sample-major swap + `iteration_order_is_active` (Task 4), `tracks=` construction incl. tracks-only datasets (Task 5), tracks-without-variants reconstruction (Task 6), mixed SVAR1 variants+re-aligned-tracks fused-kernel wiring (Task 7), `with_seqs`/`realign_tracks`/jitter guards mirroring the written path (Task 8), `max_mem` track-byte accounting (`TRACK_BYTES_PER_CELL`) + `with_insertion_fill`/`with_settings(realign_tracks=)` (Task 9), docs (Task 10, this commit). Byte-identical parity gated on `extend_to_length=False, max_jitter=None`; `extend_to_length=True`/`max_jitter>0` and non-SVAR1 mixed variants+tracks are deferred follow-ups, filed as [#375](https://github.com/mcvickerlab/GenVarLoader/issues/375)-[#384](https://github.com/mcvickerlab/GenVarLoader/issues/384). **Follow-ups landed since:** [#379](https://github.com/mcvickerlab/GenVarLoader/issues/379) (`n_batches` under-reported for the super-batched SVAR2 drives — the counter re-derived a flat `range` instead of walking the drive's `super_batch`->`batch` nesting; both now share `_super_batch_bounds`/`_batch_bounds`, so they cannot drift again); [#380](https://github.com/mcvickerlab/GenVarLoader/issues/380) (the mixed variants+tracks drive reconstructed haplotype bytes and then DISCARDED them, yielding tracks alone — it now yields the `(haplotypes, tracks)` pair the written path's `HapsTracks` returns. This was not only an API asymmetry and wasted work: with the bytes discarded, NO mixed fixture could compare the haplotype half, so streaming's mixed reconstruction was un-oracled. All 10 mixed parity tests now assert both halves byte-identical vs `Dataset[r, s]`, verified non-vacuous by a 1-bit mutation of the haplotype buffer failing every one of them); [#375](https://github.com/mcvickerlab/GenVarLoader/issues/375) Track B ([#391](https://github.com/mcvickerlab/GenVarLoader/issues/391), PR [#397](https://github.com/mcvickerlab/GenVarLoader/pull/397)) — VCF/PGEN mixed variants+tracks, byte-identical parity on both backends via a per-hap CSR the record engine exposes per window; also ported the #307 `touched_contigs` optimization to both record backends (they were materializing every contig's reference bytes per engine, measured 201.2 MB -> 0.0 MB for a plan-less engine). Review follow-ups filed: [#398](https://github.com/mcvickerlab/GenVarLoader/issues/398) (`with_len` re-aligned-track parity gap, pre-existing and shared with SVAR1, documented as a known gap), [#399](https://github.com/mcvickerlab/GenVarLoader/issues/399) (abandoned record-stream `to_iter` generator wedges the producer) ✅ **fixed**, PR [#402](https://github.com/mcvickerlab/GenVarLoader/pull/402) — `EngineState::drop` joined the producer while *holding* the GIL (it runs during Python deallocation), and `PgenWindowFiller::fill` needs the GIL to drive pgenlib, so the two waited on each other. Closing the channels first, which `drop` already did, frees a producer parked on `rx_free.recv()`/`tx_filled.send()` but not one parked inside `fill`. The join now runs under `py.detach`, the same discipline `next_batch`/`window_realign_inputs` already follow. VCF never wedged (`VcfWindowFiller` touches Python not at all) and neither did non-mixed streams (their producer parks in `rx_free.recv()`, not in `fill`) — both verified, not assumed. Regression tests: a `stream_core` unit test that drops a mid-stream engine from inside `Python::attach` (verified non-vacuous — it fails at its 30 s bound with the `detach` removed), plus 10 `tests/dataset/test_streaming_abandoned_iter{,_mixed}.py` cases guarded by a `faulthandler` C-level watchdog, since a GIL deadlock never reaches a bytecode boundary and `pytest-timeout`'s signal handler therefore never runs), [#400](https://github.com/mcvickerlab/GenVarLoader/issues/400) (fold the mixed window decode into the producer to remove the 2x decode) ✅ **fixed**, PR [#420](https://github.com/mcvickerlab/GenVarLoader/pull/420) — the drive's `RecordStreamEngine` is now the only decoder: `RecordStreamEngine.current_window_realign_inputs` reads the producer's already-filled slot (validating the requested job against the engine's current job), `_mixed_track_window` forwards `engine=`, and the private plan-less `_mixed_engine()` is deleted. Deterministic-counter gate (not wall-clock): `transpose_word_reads` on the record mixed fixture is now identical with `realign_tracks` on and off (2 word reads, both) and PGEN's `pgen_variants_decoded` likewise (5 variants, both) — the realign run was ~2x before, at 4 word reads and 10 variants. ✅ **#375 Track A (SVAR2 mixed variants+tracks)**, PR [#395](https://github.com/mcvickerlab/GenVarLoader/pull/395) — `_Svar2Realign`/`_Svar2Backend.mixed_realign_window` wired into the "sync" read drive, byte-identical to `Dataset[r, s]` for both halves. With Track A and Track B both landed, **every variant backend (SVAR1, SVAR2, VCF/BCF, PGEN) supports mixed variants+tracks**. ✅ De-duplicating the mixed drive across the `"engine"` and `"sync"` branches landed as [#394](https://github.com/mcvickerlab/GenVarLoader/issues/394), PR [#401](https://github.com/mcvickerlab/GenVarLoader/pull/401) — the ~110 duplicated lines are now one `_mixed_track_window` (per-window bounds, `mixed_realign_window`, the single `read_window`) plus one `_mixed_tracks_batch` (`regions_batch`, the written path's `base_seed` xor-reduce, `realign_batch`/`_tracks_from_intervals`) serving both drives; the three differences (`s_idx_w`, `out_lengths`, and jitter via `region_offsets=None` for SVAR2) became parameters. Deliberately sequenced after both tracks so the two independent byte-identical parity suites could serve as the safety net — before it, each copy was executed by only one suite, so a shared-logic fix applied to one would silently miss the other with both green. |
| `docs/superpowers/plans/2026-09-11-streaming-mixed-tracks-non-svar1.md` — issue [#375](https://github.com/mcvickerlab/GenVarLoader/issues/375) | Mixed variants+tracks for the **non-SVAR1** backends (SVAR2, VCF, PGEN) — the deferred half of #279's mixed path. Decomposed into a serial prelude plus two parallel tracks. **Prelude:** extract a backend-agnostic re-align seam (`_MixedRealign` bundles one window's state with the kernel that consumes it; `_MixedTracksBackend` + a `supports_mixed_tracks` flag replace an `isinstance` guard) out of the SVAR1-only `to_iter` drive — issue [#389](https://github.com/mcvickerlab/GenVarLoader/issues/389). **Track A** (Python-only): `_Svar2Realign` + `_Svar2Backend.mixed_realign_window`, wired into SVAR2's `"sync"` super-batch drive rather than the `"engine"` drive the seam first served — issue [#390](https://github.com/mcvickerlab/GenVarLoader/issues/390). **Track B**: a Rust `window_realign_inputs` pymethod + `_record_mixed_realign_window`, serving VCF and PGEN together — issue [#391](https://github.com/mcvickerlab/GenVarLoader/issues/391). The split is forced by the kernels, not by convenience: SVAR1 and the record backends share the fused `intervals_and_realign_track_fused` kernel, while SVAR2 has none and must split into `intervals_to_tracks` + `shift_and_realign_tracks_from_svar2_readbound`. | ✅ **done — prelude and both tracks landed in `streaming`.** Prelude PR [#393](https://github.com/mcvickerlab/GenVarLoader/pull/393) (`c8b3b2d3`), Track B PR [#397](https://github.com/mcvickerlab/GenVarLoader/pull/397) (`c96acbdb`), Track A PR [#395](https://github.com/mcvickerlab/GenVarLoader/pull/395) (`b15ebb0a`); the two parallel tracks were reconciled in the merge `01c0d6d9`, where every conflict turned out to be the same question — *which backends support mixed variants+tracks?* — and the answer is now all four. **Byte-identical parity vs `gvl.write()` + `Dataset[r, s]` on both halves of the `(haplotypes, tracks)` tuple, for SVAR1, SVAR2, VCF/BCF and PGEN.** De-duplicating the resulting two copies of the mixed drive was deliberately sequenced last and landed as [#394](https://github.com/mcvickerlab/GenVarLoader/issues/394), PR [#401](https://github.com/mcvickerlab/GenVarLoader/pull/401). Still open: [#398](https://github.com/mcvickerlab/GenVarLoader/issues/398) (`with_len` re-aligned-track parity gap, pre-existing), [#399](https://github.com/mcvickerlab/GenVarLoader/issues/399) (abandoned record-stream `to_iter` generator wedges the producer) ✅ **fixed**, PR [#402](https://github.com/mcvickerlab/GenVarLoader/pull/402) — `EngineState::drop` joined the producer while *holding* the GIL (it runs during Python deallocation), and `PgenWindowFiller::fill` needs the GIL to drive pgenlib, so the two waited on each other. Closing the channels first, which `drop` already did, frees a producer parked on `rx_free.recv()`/`tx_filled.send()` but not one parked inside `fill`. The join now runs under `py.detach`, the same discipline `next_batch`/`window_realign_inputs` already follow. VCF never wedged (`VcfWindowFiller` touches Python not at all) and neither did non-mixed streams (their producer parks in `rx_free.recv()`, not in `fill`) — both verified, not assumed. Regression tests: a `stream_core` unit test that drops a mid-stream engine from inside `Python::attach` (verified non-vacuous — it fails at its 30 s bound with the `detach` removed), plus 10 `tests/dataset/test_streaming_abandoned_iter{,_mixed}.py` cases guarded by a `faulthandler` C-level watchdog, since a GIL deadlock never reaches a bytecode boundary and `pytest-timeout`'s signal handler therefore never runs), [#400](https://github.com/mcvickerlab/GenVarLoader/issues/400) (fold the mixed window decode into the producer to remove the 2x decode) ✅ **fixed**, PR [#420](https://github.com/mcvickerlab/GenVarLoader/pull/420) — the drive's `RecordStreamEngine` is now the only decoder: `RecordStreamEngine.current_window_realign_inputs` reads the producer's already-filled slot (validating the requested job against the engine's current job), `_mixed_track_window` forwards `engine=`, and the private plan-less `_mixed_engine()` is deleted. Deterministic-counter gate (not wall-clock): `transpose_word_reads` on the record mixed fixture is now identical with `realign_tracks` on and off (2 word reads, both) and PGEN's `pgen_variants_decoded` likewise (5 variants, both) — the realign run was ~2x before, at 4 word reads and 10 variants. One correction to the prelude's own description above: the capability guard is enforced at **runtime** by the `isinstance` narrowing only — the `unsafe-overlap` static half was re-tested during #394 (swapped return tuple and wrong parameter dtype, before and after the refactor) and pyrefly reports nothing for any of them with this repo's current config. |
