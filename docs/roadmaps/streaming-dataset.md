...
| `docs/superpowers/plans/2026-07-17-streaming-vcf-pgen-backends.md` — issue [#276](https://github.com/mcvickerlab/GenVarLoader/issues/276) | VCF backend / PGEN backend. Spec: `docs/superpowers/specs/2026-07-17-streaming-vcf-pgen-backends-design.md` | 🚧 Tasks 1-7 done (generic `generate_batch_core`/`DecodedWindow` transpose/`RecordStreamEngine` core/`VcfWindowFiller`/Python `#[new]`+`next_batch` FFI seam/`_VcfBackend` Python wiring/variant-table parity gate — Task 7's `RecordStreamEngine.debug_decode_window` proved the streamed table byte-equals `gvl.write`'s stored table on first try, no config reconciliation needed); Task 8 (end-to-end haplotype byte parity) next |
