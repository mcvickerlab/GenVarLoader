...
`"variants"`/`"variant-windows"`/`"reference"` output, `min_af`/`max_af`, and `var_fields` are **not yet implemented** for `StreamingDataset` (Wave B, issue [#304](https://github.com/mcvickerlab/GenVarLoader/issues/304)); `with_seqs` raises `NotImplementedError` for any other kind. See the `genvarloader` skill for the full scope.
