...
Yes, for haplotype output from a `.svar` (SparseVar) or `.svar2` variant source: [`gvl.StreamingDataset`](api.md#genvarloader.StreamingDataset) reconstructs haplotype batches directly from a live `.svar`/`.svar2` store, with no [`gvl.write()`](api.md#genvarloader.write) call and no `.gvl` directory written to disk.
