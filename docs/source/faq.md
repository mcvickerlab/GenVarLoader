...
**`var_fields` applies only to `with_seqs("variants")`** (Wave B PR-B3a, issue [#304](https://github.com/mcvickerlab/GenVarLoader/issues/304)) — combining `with_settings(var_fields=[...])` with any other output kind raises `NotImplementedError`, but not until **iterate time** (when `to_iter` is called); `with_settings` itself accepts the combination and only the subsequent iteration fails. This is **stricter** than the written path, which also allows `var_fields` on `"variant-windows"` (streaming doesn't support that output kind yet). The requestable set (`available_var_fields`) is backend-derived rather than read from an on-disk schema: SVAR1 offers its numeric index columns (e.g. `AF`) plus `ref`; VCF/BCF offers every numeric INFO field the live header declares plus `ref`; PGEN offers only `ref` (no INFO path). Not everything advertised is servable yet — `servable_var_fields` narrows that set (e.g. SVAR1's `AF` index column is advertised but not yet gatherable by the streaming engine); unlike the output-kind guard above, this check DOES fire eagerly, at `with_settings` time, raising `NotImplementedError` (deferred to PR-B3b). For a VCF/BCF source, streaming's `available_var_fields` can be a strict superset of a written `Dataset`'s for the same source: `gvl.write()` only ever persists one numeric INFO column (`AF`) into a queryable written schema, so any other declared numeric INFO field (e.g. an Integer `DP`) is streaming-only — requesting it from a written `Dataset` raises `ValueError`. See [dataset.md](dataset.md) for the full table and caveat.
