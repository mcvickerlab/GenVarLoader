...
def gen_table(n_samples: int, ipp: int, contig_len: int = 1_000_000, seed: int = 0) -> pl.DataFrame:
...
def gen_queries(n_regions: int, contig_len: int = 1_000_000, seed: int = 1) -> tuple[np.ndarray, np.ndarray]:
    rng = np.random.default_rng(seed)
