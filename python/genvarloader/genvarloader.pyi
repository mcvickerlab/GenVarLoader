...
def count_intervals(
    ...
    Args:
        paths (Sequence[str | Path]): Paths to BigWig files.
        contig (str): Contig name.
        starts (NDArray[int32]): Start positions.
        ends (NDArray[int32]): End positions.

    Returns:
        n_per_query (NDArray[int32]): Shape = (regions, samples) Number of intervals per query.
    """
...
def bigwig_intervals(
    ...
    Args:
        ...
        starts (NDArray[int32]): Start positions.
        ends (NDArray[int32]): End positions.
        offsets (NDArray[int64]): Offsets corresponding to the returned interval data of shape
            (regions, samples). Can be computed from the number of intervals per query, e.g. with
            the count_intervals function.

    Returns:
        coordinates (NDArray[uint32]): Shape = (intervals, 2) Coordinates.
        values (NDArray[float32]): Shape = (intervals) Values.
    """
