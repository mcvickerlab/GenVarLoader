...
def case_snps():
    ...
    v_starts = np.array([1, 2, 3], np.int32)
    ilens = np.array([0, 0, 0], np.int32)
    query_start = 0
    query_end = 4

    desired = np.ones(3, np.bool_)

    return variant_idxs, ccfs, v_starts, ilens, query_start, query_end, desired
...
def case_indels_0():
    ...
    variant_idxs = np.array([0, 1, 2], np.int32)
    ccfs = np.array([0.3, 0.2, 0.1], np.float32)
    v_starts = np.array([1, 3, 5], np.int32)
    ilens = np.array([-3, 1, 0], np.int32)
    query_start = 0
    query_end = 8
    ...
    for g in range(n_groups):
        ...
        v_ends = v_starts - np.minimum(0, ilens[variant_idxs[groups == g]]) + 1
...
def case_del_span_start():
    ...
    variant_idxs = np.array([0, 1, 2], np.int32)
    ccfs = np.array([0.3, 0.2, 0.1], np.float32)
    v_starts = np.array([0, 1, 4], np.int32)
    ilens = np.array([-2, 0, 0], np.int32)
    query_start = 2
    query_end = 12
...
@parametrize_with_cases(
    "variant_idxs, ccfs, v_starts, ilens, query_start, query_end, desired",
    cases=".",
)
