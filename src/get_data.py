...
def get_read_depth(in_bam, chrom_list=None, chrom_lens=None):
    ...
    with tempfile.NamedTemporaryFile() as temp_file:
        ...
        for chrom in chrom_list:
            ...
            pysam.depth("-r", chrom, "-o", temp_file.name, in_bam, catch_stdout=False)
            ...
            out_array = np.zeros(chrom_lens[chrom], dtype=np.uint8) # make output array
            out_array[pos_array] += count_array
            
            depth_dict[chrom] = out_array
            
            print(f"Counted {chrom} read-depth in {timeit.default_timer() - start_chrom:.2f} seconds!")

    print(f"Processed read-depth data in {timeit.default_timer() - start_time:.2f} seconds!")
    return depth_dict
...
def get_allele_coverage(in_bam, chrom_list=None):
    ...
    with AlignmentFile(in_bam, "r") as bam:
        ...
        for chrom in chrom_list:
            ...
            print(f"Counted {chrom} coverage in {timeit.default_timer() - start_chrom:.2f} seconds!")

    print(f"Processed allele coverage in {timeit.default_timer() - start_time:.2f} seconds!")
